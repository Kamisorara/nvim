...
utils.about = function()
    ...
    render("", 1)
    render("A beautiful, powerful and highly customizable neovim config based on IceNvim.", 2)
    render("", 3)
...
utils.check_icons = function()
    ...
    local new_line = line()
    local row
    for name, icon in require("core.utils").ordered_pair(Kamisora.symbols) do
...
utils.colorscheme = function(colorscheme)
    Kamisora.colorscheme = colorscheme
...
utils.select_colorscheme = function()
    ...
    local function picker(opts)
        ...
        local colorschemes = Kamisora.colorschemes
        local results = {}
